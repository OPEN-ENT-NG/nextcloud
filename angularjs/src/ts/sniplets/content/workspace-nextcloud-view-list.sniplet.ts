...
export class NextcloudViewList implements INextcloudViewList {
    ...
    orderByField (fieldName: string, desc?: boolean): void {
        ...
        this.vm.documents = this.vm.documents.sort((a: SyncDocument, b: SyncDocument) => {
            if (this.orderField === 'lastModified') {
                ...
                return this.orderDesc ? a[fieldName].localeCompare(b[fieldName]) : b[fieldName].localeCompare(a[fieldName]);
            }
            return 0;
        })
    }
